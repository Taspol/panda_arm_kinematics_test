...
//---------Singularity test---------
  ...
  int skip_count = 0;
  bool test = true;
  std::vector<std::tuple<double,double,double>> available_angle;
  if(command == "0"){
 //   std::cin.ignore();
    //std::getline(std::cin,command);
    for(double i = -1.483 ; i <= -1.483 ; i+=0.01){
       for(double j = -1.283 ; j <= 0.802 ; j+=0.01){
          for(double k = -1.570 ; k <= 1.570 ; k+=0.01){
	     if(TEST("TEST_ANGLE",-k,-j,-i,true,move_group_interface,"JOINT")) break;
	     available_angle.push_back(std::make_tuple(k,j,i));
	  }
       }
